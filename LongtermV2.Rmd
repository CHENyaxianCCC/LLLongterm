...
plot_ecosystem <- ggplot(data = ecosystem_percentages) +
  geom_bar(aes(x = 3, y = percentage, fill = Ecosystem), stat = "identity", width = 1.2, color = "white", size = 0.8) + 
...
plot_Fig1 <- (site_plot2 / (plot_years + plot_ecosystem + plot_layout(ncol = 2, widths = c(3.8, 1.2)))) +
                 plot_annotation(tag_levels = "a") +
                 plot_layout(heights = c(6, 4))
plot_Fig1
#save_agu_plot("plot_Fig1.png", width = 8, height = 5.5)
```
...
plot_lm_results_sub <- lm_results %>%
  ...
  geom_point(data = lm_results_mean_trend,
             aes(x = tm_slope_mean, y = Rs_slope_mean, color = trend_mean),  
             size = 3.5) +
  scale_color_manual(values = c("Decrease or No Change" = "skyblue",  
                                "Significant Increase" = "#EF797E",
                                "Not Significant but Increase" = "#FDCCA5",
                                "Decrease or No Change Mean" = "#4285F4",  
...
#knitr::kable(lm_sub_summary , caption = "Summary of Lm Result ")
```
...
results <- plot_trend(lm_results_decrease, "Decrease or No Change")
lm_sub_summary$Comments[lm_sub_summary$trend == "Decrease or No Change"] <- paste(results$equation, results$r_squared, results$p_value)

results <- plot_trend(lm_results_not_significant, "Not Significant but Increase")
lm_sub_summary$Comments[lm_sub_summary$trend == "Not Significant but Increase"] <- paste(results$equation, results$r_squared, results$p_value)

results <- plot_trend(lm_results_significant_increase, "Significant Increase")
lm_sub_summary$Comments[lm_sub_summary$trend == "Significant Increase"] <- paste(results$equation, results$r_squared, results$p_value)

print(lm_sub_summary)
#library(gridExtra)
#library(ggplot2)
#lm_sub_summary_plot <- tableGrob(lm_sub_summary)
#ggsave(filename = "lm_sub_summary.png", plot = lm_sub_summary_plot)
...
faceted_plot <- lm_results_combined %>%
  ...
  # 显示回归结果
geom_text(aes(x = -0.2, y = 0, label = label), size = 4, col = "black") +
...
# 首次出现显著的年份直方图
median_Positive_first_change <- median(Positive_first_change_matrix[!is.na(Positive_first_change_matrix)], na.rm = TRUE)
Positive_first_change_data <- data.frame(Year = Positive_first_change_matrix[!is.na(Positive_first_change_matrix)])
ggplot(Positive_first_change_data, aes(x = Year)) + 
  geom_histogram(bins = 30, fill = "gray", col = "black") +
  geom_vline(xintercept = median_Positive_first_change, color = "red") +
  ylab("Frequency") + 
  xlab("Year") + 
  ggtitle("Histogram of First Year of Significance (Slope > 0)") -> plot_Positive_first_change
 plot_Positive_first_change
...
# 设置 RasterLayer 对象的值
#values(r_1) <- first_change_matrix
#values(r_2) <- Positive_first_change_matrix
#values(r_3) <- combined_matrix

# 设置坐标参考系统和范围
#crs(r_1) <- CRS("+proj=longlat +datum=WGS84")  extent(r_1) <- extent(-180, 180, -90, 90)
#crs(r_2) <- CRS("+proj=longlat +datum=WGS84")  extent(r_2) <- extent(-180, 180, -90, 90)
#crs(r_3) <- CRS("+proj=longlat +datum=WGS84")  extent(r_3) <- extent(-180, 180, -90, 90)

# 保存投影后的栅格数据
#writeRaster(r_1, filename = "D:/git/Github/Longterm/tifdata/first_change.tif",format = "GTiff", overwrite = TRUE, options = "INTERLEAVE=PIXEL")
#writeRaster(r_2, filename = "D:/git/Github/Longterm/tifdata/Positive_first_change.tif",format = "GTiff", overwrite = TRUE, options = "INTERLEAVE=PIXEL")
#writeRaster(r_3, filename = "D:/git/Github/Longterm/tifdata/combined_change.tif",format = "GTiff", overwrite = TRUE, options = "INTERLEAVE=PIXEL")
```
...
# 首次出现显著的年份直方图
median_fuzz_Positive_first_change <- median(fuzz_Positive_first_change_matrix[!is.na(fuzz_Positive_first_change_matrix)], na.rm = TRUE)
fuzz_Positive_first_change_data <- data.frame(Year = fuzz_Positive_first_change_matrix[!is.na(fuzz_Positive_first_change_matrix)])
ggplot(fuzz_Positive_first_change_data, aes(x = Year)) + 
  geom_histogram(bins = 30, fill = "gray", col = "black") +
  geom_vline(xintercept = median_Positive_first_change, color = "red") +
  ylab("Frequency") + 
  xlab("Year") + 
  ggtitle("Histogram of First Year of Significance (Slope > 0)") -> plot_fuzz_Positive_first_change
 plot_fuzz_Positive_first_change
...
# 设置 RasterLayer 对象的值
#values(r_4) <- fuzz_first_change
#values(r_5) <- fuzz_Positive_first_change_matrix
#values(r_6) <- fuzz_combined_matrix
# 设置投影和范围
...
#writeRaster(r_4, filename = "D:/git/Github/Longterm/tifdata/fuzz_first_change.tif", format = "GTiff", overwrite = TRUE, options = "INTERLEAVE=PIXEL")
#writeRaster(r_5, filename = "D:/git/Github/Longterm/tifdata/fuzz_Positive_first_change.tif", format = "GTiff", overwrite = TRUE, options = "INTERLEAVE=PIXEL")
#writeRaster(r_6, filename = "D:/git/Github/Longterm/tifdata/fuzz_combined_change.tif", format = "GTiff", overwrite = TRUE, options = "INTERLEAVE=PIXEL")

 plot_grid(plot_combined_matrix, plot_Positive_first_change , plot_fuzz_combined_matrix, plot_fuzz__Positive_first_change,  
          labels = c("a", "b", "c", "d"))
